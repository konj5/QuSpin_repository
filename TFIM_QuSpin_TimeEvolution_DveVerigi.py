...
def exactDiag(N:int, hx:float, J:float, JT:float) -> tuple:

    #Veriga 1 [0 : N], veriga 2 [N:2N]
    hx_field = [[-hx, i] for i in range(2*N)] #Transverzalno polje
    ...
    static_list = [["zz", J_interaction], ["x", hx_field], ["zz", JT_interaction], ["z", hz_field]]
    dynamic_list = []
...
def timeEvolution(N:int, J:float, hxdrive:object, JTdrive:object):
    ...
    hx_field2 = [[-1000, N+i] for i in range(N)] #Transverzalno polje
    ...
    dynamic_list = [["x",hx_field1,hxdrive.drive, ["garbageparameter"]], ["zz",JT_interaction,JTdrive.drive,["garbageparameter"]] ]
    ...
    groundstate = exactDiag(N = N, J = J, hx = hxdrive.hx0, JT = 0)[1][:,0]

    #groundstate = [0 if i not in (0,2**N-1) else 1/np.sqrt(2) for i in range(2**N)]

    return(H.evolve(v0=groundstate, t0 = t0, times=np.linspace(t0,tend,100)))
...
class hxDrive:
    ...
    def __init__(self, t0:float,tend:float, hx0:float, hxend:float,a:float) -> None:
        self.tmax = tend
        self.tend = tend
        self.t0 = t0
        self.tstart = t0
...
y1s = [hxDrive(0,10,10,0,10).drive(t,"trash") for t in ts]
y2s = [JTDrive(1,10,1,hxDrive(0,10,10,0,10)).drive(t,"trash") for t in ts]
